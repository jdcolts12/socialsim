...
export function generateFeedback(messages: { role: 'user' | 'assistant'; content: string }[]): Feedback {
  ...
  const professionalism = Math.min(95, Math.round(50 + Math.min(avgLength / 3, 20) + (userMessages.length >= 5 ? 15 : 0)));
  ...
  return {
    ...
    professionalism: Math.max(40, Math.min(95, professionalism)),
